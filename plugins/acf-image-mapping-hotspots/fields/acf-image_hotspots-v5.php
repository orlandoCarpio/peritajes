	...
	*/
	...
	function render_field( $field ) {
		...
		*/
		...
		$xy_pair_group       = explode( '-', $field_value );
...
HTML;
		...
		$cont=count($xy_pair_group);
       if($xy_pair_group[0]!="")
			$contIn=-(count($xy_pair_group)-1);		
		else
			$contIn=-(count($xy_pair_group));
		if($cont!=0 && $xy_pair_group[0]!=""){
		 foreach ($xy_pair_group as &$valor) {
			$xy_pair       = explode( ",", $valor );
			$acc=$cont+$contIn;
			$contIn++;
				$x = $xy_pair[0];
				$y = $xy_pair[1];
		...
		echo <<<HTML
<span data-left="$x"data-top="$y" style="left:$x;top:$y;">$acc</span>
HTML;
}}
		...
		echo <<<HTML
			...
			<!-- XY Coordinate Pair -->
					<input class="$this->name-input" id="input-map"type="text" name="$field_name" value="$field_value" />
HTML;
